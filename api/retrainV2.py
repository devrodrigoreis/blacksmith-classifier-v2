...
nltk.download('averaged_perceptron_tagger_eng', quiet=True)  # Added to fix the error
...
def augment_text(text, aug):
    ...
    try:
        ...
        logging.warning(f"Augmentation failed for text '{text}': {str(e)}. Returning original text.")
        return text

def augment_small_classes(data, aug, min_samples=50):
...
def main():
    ...
    data = augment_small_classes(data, aug, min_samples=50)
