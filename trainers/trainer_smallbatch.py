...
MAX_MEMORY_PERCENT = config.get('memory', {}).get('max_memory_percent', 99.0)
...
def main():
    ...
    logging.info(f"Data loaded and preprocessed, total samples: {total_lines}")

    label_encoder = LabelEncoder()
    data['full_category'] = label_encoder.fit_transform(data['full_category'])
    logging.info(f"Encoded {len(label_encoder.classes_)} unique categories")

    logging.info("Balancing classes with oversampling...")
    data = filter_small_classes(data, min_samples=10)
    ros = RandomOverSampler(random_state=42, sampling_strategy='not majority')
    resample_result = ros.fit_resample(data[['product_name']], data['full_category'])
    if len(resample_result) == 2:
        X_resampled, y_resampled = resample_result
    else:
        X_resampled, y_resampled, _ = resample_result
    data_resampled = pd.DataFrame({'product_name': X_resampled['product_name'], 'full_category': y_resampled})
    logging.info(f"Classes balanced, resampled dataset size: {len(data_resampled)}")
    ...
    # REDUCED BATCH SIZE AND ADJUSTED GRADIENT ACCUMULATION
    training_args = TrainingArguments(
        ...
        fp16=True,
        gradient_accumulation_steps=16,  # increased to keep effective batch size
