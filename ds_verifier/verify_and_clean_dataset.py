...
def load_dataset(file_path: str, chunk_size: int = 10000) -> Optional[Iterator[pd.DataFrame]]:
    try:
...
# Load categories for improved misclassification detection
def load_categories(categories_path: str = 'data/categories.csv') -> List[str]:
    try:
        categories_df = pd.read_csv(categories_path, encoding='utf-8')
        category_paths = categories_df['category_name'].tolist()
        print(f"Loaded {len(category_paths)} categories from {categories_path}.")
        return category_paths
    except FileNotFoundError:
        print(f"Error: '{categories_path}' not found. Limited misclassification detection will be used.")
        return []
    except Exception as e:
        print(f"Error loading categories CSV: {e}")
        return []

# Compute word overlap score between product name and category
def get_word_overlap(product_name: str, category: str) -> int:
    product_words: Set[str] = set(re.findall(r'\w+', product_name.lower()))
    category_words: Set[str] = set(re.findall(r'\w+', category.lower()))
    return len(product_words & category_words)

# Fix inconsistencies in a chunk (excluding misclassifications)
def fix_inconsistencies(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    df = df.copy()
    fixes_applied = []
...
# Check for inconsistencies in a chunk (tagging all misclassifications)
def check_inconsistencies(df: pd.DataFrame, category_paths: List[str]) -> pd.DataFrame:
    inconsistencies = []

    def add_inconsistency(pid, product_name, category, issue, suggested_category=None):
        entry = {
            'pid': pid,
        ...
        if suggested_category:
            entry['suggested_category'] = suggested_category
    ...
    # 4. Tag specific misclassifications
    for _, row in df.iterrows():
        ...
        if 'dobradiça' in product_name and 'caixas d\'água' in category:
            add_inconsistency(
                row['pid'], row['product_name'], row['product_category_tree'],
                "Misclassification: Hinges under water tanks",
                suggested_category="Material de construção>>Portas e Janelas>>Dobradiças"
            )
        ...
        if 'graxa calçados' in product_name and 'automotivo' in category:
            add_inconsistency(
                row['pid'], row['product_name'], row['product_category_tree'],
                "Misclassification: Shoe grease in automotive",
                suggested_category="Vestuário>>Roupas e acessórios>>Sapatos"
            )
        ...
        if 'torneira p/haste pulverização' in product_name and 'torneiras maquina de lavar e tanque' in category:
            add_inconsistency(
                row['pid'], row['product_name'], row['product_category_tree'],
                "Misclassification: Spray faucet in household faucets",
                suggested_category="Pet e Vet e Agro>>Irrigação>>Torneiras de Pulverização"
            )

    # 5. Tag unknown misclassifications using categories.csv
    if category_paths:
        for _, row in df.iterrows():
            product_name = row['product_name'].lower()
            current_category = row['product_category_tree']
            current_score = get_word_overlap(product_name, current_category)

            max_score = current_score
            suggested_category = current_category

            for cat in category_paths:
                score = get_word_overlap(product_name, cat)
                if score > max_score:
                    max_score = score
                    suggested_category = cat

            if suggested_category != current_category and max_score > current_score + 2:
                add_inconsistency(
                    row['pid'], row['product_name'], current_category,
                    f"Potential misclassification: Better match with '{suggested_category}' (score: {max_score} vs {current_score})",
...
def save_output(df: pd.DataFrame, output_file: str, output_type: str = 'inconsistencies') -> None:
    if not df.empty:
...
def main():
    ...
    # Load categories
    category_paths = load_categories(categories_path)
    ...
    for chunk in chunks:
        ...
        # Check inconsistencies (tagging all misclassifications)
        inconsistencies = check_inconsistencies(cleaned_chunk, category_paths)
